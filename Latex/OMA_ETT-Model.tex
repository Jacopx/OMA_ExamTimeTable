...
We have starting defining 2 main boolean variables. The first is related to the exam in time slots:
\begin{equation}
  \begin{gathered}
...
Out model is designed to MINIMIZE the following expression:
\begin{equation}
  \begin{gathered}
  ...
  \end{gathered}
\end{equation}
the only problem of this equation is that is not a linear function (due to the product of two decision variables). We nedd to linearize it:
\begin{equation}
  \begin{gathered}
  ...
  \end{gathered}
\end{equation}
this is not enough, we need to define also other constratins in the equation number \ref{eq:linearize}.

\subsection{Constrains}
There are two main constratins that need to be defined for generating feasible solution, the first is the one related to the possibility to define only once an exam during the examination period:
\begin{equation}
  \begin{gathered}
...
\begin{equation}
  ...
  \end{gathered}
\end{equation}
The following constratins are defined due to \textbf{linearize the objective function}:
\begin{equation}
  \begin{gathered}
